...
import type { ProcessLine } from "@/features/instances/hooks/use-instance-process";
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <Button variant="ghost" size="icon-xs" onClick={onClear} disabled={lines.length === 0}>
          <Trash2 />
        </Button>
      </div>

      <ScrollArea className="h-48 w-full rounded-md border border-border/30 bg-black/40">
        <div className="space-y-0.5 p-3 font-mono text-[0.7rem] leading-relaxed">
          {lines.length === 0 ? (
