...
pub fn load_instance_settings(data_dir: &Path) -> Result<InstanceSettings, String> {
    ...
    let contents =
        std::fs::read_to_string(&path).map_err(|e| format!("Failed to read instance settings: {e}"))?;
    let settings: InstanceSettings =
        serde_json::from_str(&contents).map_err(|e| format!("Invalid instance settings JSON: {e}"))?;
...
pub fn save_instance_settings(data_dir: &Path, settings: &InstanceSettings) -> Result<(), String> {
    crate::instance_data::ensure_instance_data_layout(data_dir)?;
    let path = instance_settings_path(data_dir);
    let normalized = crate::instance_settings::defaults::normalize_instance_settings(settings.clone());
...
pub fn persist_settings_from_manifest(data_dir: &Path, settings: &InstanceSettings) -> Result<(), String> {
    save_instance_settings(data_dir, settings)?;
    write_launch_settings_overlay(data_dir, settings)
}

pub fn write_launch_settings_overlay(data_dir: &Path, settings: &InstanceSettings) -> Result<(), String> {
    crate::instance_data::ensure_instance_data_layout(data_dir)?;
